...
class ReportGenerator:
    ...
    def generate(self, recon_results, ai_analysis="", target=""):
        ...
        # ── Guided Remediation (Section 1.6) ──────────────────
        # Each finding gets a structured remediation block: explanation, CWE/OWASP,
        # general steps and a generic secure-pattern snippet. Rule-based (no AI) so
        # report generation is fast and works offline.
        self._add_remediation_section(story, recon_results, target)
    ...
    def _add_remediation_section(self, story, recon_results, target):
        """Append a Guided Remediation block per extracted finding (Section 1.6)."""
        try:
            from modules.intel.findings_extract import extract_findings_from_recon
            from modules.intel.remediation import generate_remediation
        except Exception:
            return

        try:
            findings = extract_findings_from_recon(recon_results)
        except Exception:
            findings = []
        if not findings:
            return
        ...
        story.append(PageBreak())
        story.append(Paragraph("Guided Remediation", self.heading_style))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#00AAFF')))
        story.append(Paragraph(
            "For every finding below: a plain-language explanation, the relevant "
            "CWE/OWASP category, general remediation steps and a generic secure-pattern "
            "snippet. Guidance is best-practice and defensive — it is not a host-specific "
            "patch and contains no exploit code.", self.body_style))
        ...
        for f in findings[:25]:
            try:
                ...
                continue
            story.append(Spacer(1, 0.12*inch))
            story.append(Paragraph(esc(block["title"]), self.subheading_style))
            story.append(Paragraph(f"<b>{esc(block['severity_context'])}</b>", self.body_style))
            story.append(Paragraph(f"<b>CWE:</b> {esc(block['cwe'])}", self.body_style))
            story.append(Paragraph(f"<b>OWASP:</b> {esc(block['owasp'])}", self.body_style))
            story.append(Paragraph(esc(block["explanation"]), self.body_style))
            story.append(Paragraph("<b>Remediation steps:</b>", self.body_style))
            for i, step in enumerate(block["remediation_steps"], 1):
                story.append(Paragraph(f"{i}. {esc(step)}", self.body_style))
            story.append(Paragraph("<b>Secure pattern (generic):</b>", self.body_style))
            for line in block["secure_snippet"].split('\n'):
                story.append(Paragraph(esc(line) or "&nbsp;", self.code_style))
            if block.get("exploit_reference"):
                story.append(Paragraph(f"<b>{esc(block['exploit_reference'])}</b>", self.body_style))
