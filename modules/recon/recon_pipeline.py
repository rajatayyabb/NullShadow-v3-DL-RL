...
class ReconPipeline:
    ...
    def _whois(self, target):
        ...
        # Method 1: RDAP via the shared IANA-bootstrap helper (Phase 0.2).
        # This resolves non-generic TLDs (.fr, .br, .edu, etc.) that the old
        # hardcoded com/net/org/io dict could not.
        try:
            from modules.intel.rdap_lookup import rdap_lookup
            rdap = rdap_lookup(domain)
            if rdap:
                # Re-key to this module's lower-case convention.
                result = {}
                if rdap.get("Domain Name"):   result["domain"] = rdap["Domain Name"]
                if rdap.get("Status"):        result["status"] = rdap["Status"]
                if rdap.get("Created"):       result["created"] = rdap["Created"]
                if rdap.get("Expires"):       result["expires"] = rdap["Expires"]
                if rdap.get("Registrar"):     result["registrar"] = rdap["Registrar"]
                if rdap.get("Name Servers"):  result["nameservers"] = rdap["Name Servers"]
                return result
        except Exception:
            pass
